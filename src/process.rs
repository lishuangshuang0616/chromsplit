...
pub fn process_gxf(ingxf: &str, region: &str, outgxf: &str) -> Result<()> {
    ...
    for line in reader.lines() {
        ...
        reg.entry(chr.to_string()).or_default().insert(start, end);
    }
    ...
    for line in reader.lines() {
        ...
        if let Some(chr_regions) = reg.get(chr) {
            for (&s, &e) in chr_regions.iter() {
                ...
                if end > e {
                    return Err(anyhow!(
                        "Feature crosses split region boundary {}:{}-{}:\n{}",
                        chr,
                        s,
                        e,
                        line
                ...
                if chr_regions.len() == 1 {
                    ...
                    writeln!(
                        out_file,
                        "{}\t{}\t{}\t{}\t{}\t{}",
                        chr, source, type_, new_s, new_e, rest
                    )?;
                } else {
        ...
        if (processed_count + skipped_count) % 100000 == 0 {
            eprintln!(
                "Processed {} annotation records, skipped {}",
    ...
    eprintln!(
        "Annotation processing completed: processed {} records, skipped {}",
        processed_count, skipped_count
    );
    Ok(())
}
...
pub fn process_with_cut_sites(fa_file: &str, cut_site_file: &str, outpref: &str) -> Result<()> {
    ...
    let mut out_fa = std::io::BufWriter::new(
        std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(format!("{}.fa", outpref))?,
    );
    let mut out_txt = std::io::BufWriter::new(
        std::fs::OpenOptions::new()
            .create(true)
            .append(true)
    ...
    for line in reader.lines() {
        ...
        if line.starts_with('>') {
            ...
            if !current_id.is_empty() && !current_sequence.is_empty() {
                process_sequence_with_cut_sites(
                    &current_id,
                    &current_sequence,
                    &cut_sites,
                    &mut out_fa,
...
fn process_sequence_with_cut_sites(
    ...
    if let Some(regions) = cut_sites.get(id) {
        eprintln!(
            "Processing chromosome {} with {} predefined regions",
            id,
            regions.len()
        );
